...
static const char *FETCH_EXPIRED_SQL =
	"SELECT id, shard_key, action FROM timers.timers "
...
/*
 * Execute a single timer action inside a subtransaction.
 * Returns true on success, false on failure (with error_msg set).
 */
static bool
execute_timer_action(const char *action, char **error_msg)
{
	...
	PG_TRY();
	{
		int ret = SPI_execute(action, false, 0);
		if (ret < 0)
	...
	PG_TRY();
	{
		...
		if (nprocessed > 0)
		{
			...
			for (i = 0; i < nprocessed; i++)
			{
				...
				bool		ok = execute_timer_action(actions[i], &error_msg);
			...
			pfree(ids);
			pfree(shard_keys);
			/* actions freed with memory context */
		}
